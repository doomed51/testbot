...
RacBot.prototype.run = function() {
	...
	console.log('running on start commands...');
...
//_onStart > connect to db	!!! Not Used atm !!!
...
RacBot.prototype._welcomeMessage = function () {
    this.postMessageToChannel(this.channels[0].name, 'Hi guys, roundhouse-kick anyone? use `' + this.name + '` to invoke me!',
        {as_user: true});
};
...
RacBot.prototype._replyWithJoke = function (originalMessage){
	...
	console.log("posting messge to channel " + originalMessage.text);
	self.postMessageToChannel(channel.name, originalMessage.text.toUpperCase(), {as_user:true});
	/*
